...
def _normalize_window(past: np.ndarray, window: np.ndarray) -> np.ndarray:
  """Z-score `window` using mean/std of `past` (context-only stats).

  Some ETT variates (e.g. ETTm2 ``HULL``) can be exactly zero for the whole
  context window; std and peak-to-peak are then ~0. Dividing by a tiny epsilon
  (1e-6, 1e-9) still blows up normalized targets. If context is (near-)flat,
  we only center by ``mu`` and use unit scale (same protocol as a constant
  series in the base runner).
  """
  mu = float(np.mean(past))
  std = float(np.std(past))
  ptp = float(np.ptp(past))
  if ptp < 1e-12 or std < 1e-12:
    return window - mu
  scale = max(std, 1e-9, 1e-6 * max(1.0, abs(mu)))
  return (window - mu) / scale
