#!/usr/bin/env python3
"""
RoPE ablation: compare ICF-TRAINED inference with use_nope=True vs False.

Identical to rope_ablation.py except the model is loaded from a fine-tuned
checkpoint (ICF continued-pretraining on BTC-USD) rather than the raw base
pretrained weights.

Checkpoint: results/icf_trained/btc_usd_icf.pt  (or --checkpoint <path>)
"""
import sys
import json
import math
...
DEFAULT_CHECKPOINT = str(ROOT / "results" / "icf_trained" / "btc_usd_icf.pt")
...
def load_trained_icf(checkpoint_path: str) -> TimesFM_ICF_torch:
    """Load ICF-trained checkpoint weights over the pretrained base architecture."""
    icf = TimesFM_ICF_torch.from_pretrained_base("google/timesfm-2.5-200m-pytorch")
    if not os.path.exists(checkpoint_path):
        raise FileNotFoundError(
            f"ICF trained checkpoint not found: {checkpoint_path}\n"
            "Run benchmark/finetune_crypto_icf.py first to produce it."
        )
    device = icf.model.sep_token.device
    ckpt = torch.load(checkpoint_path, map_location=device, weights_only=True)
    icf.model.load_state_dict(ckpt["model"])
    icf.model.train(False)
    print(f"  Loaded ICF-trained weights from: {checkpoint_path}")
    return icf


def run_icf_backtest(prices: np.ndarray, use_nope: bool, label: str, checkpoint_path: str) -> dict:
    patch = 32
    out_patch = 128
    ...
    icf = load_trained_icf(checkpoint_path)
    ...
    print(f"{label:30s}  MSE={agg_mse:>14,.0f}  MAE={agg_mae:>8,.1f}  windows={len(results)}")
    return {
        "label": label,
        "use_nope": use_nope,
        "mse": agg_mse,
...
def plot_comparison(data_nope, data_rope, out_dir):
    ...
    axes[0].set_title('ICF Trained MSE: noPE vs RoPE', fontweight='bold')
    axes[0].set_xticks(x)
    ...
    axes[1].set_title('ICF Trained MAE: noPE vs RoPE', fontweight='bold')
    axes[1].set_xticks(x)
    ...
    plt.tight_layout()
    path1 = os.path.join(out_dir, "rope_ablation_trained_aggregate.png")
    ...
    ax.plot(np.arange(n), nope_windows[:n], marker='o', markersize=4,
            label='ICF Trained (noPE)', color='#d62728', alpha=0.7, linewidth=1.5)
    ax.plot(np.arange(n), rope_windows[:n], marker='s', markersize=4,
            label='ICF Trained (RoPE)', color='#2ca02c', alpha=0.7, linewidth=1.5)
    ax.set_xlabel('Test Window Index', fontweight='bold')
    ax.set_ylabel('MSE per Window', fontweight='bold')
    ax.set_title('Per-Window MSE: ICF Trained — noPE vs RoPE', fontweight='bold')
    ax.legend(frameon=True, fancybox=False, edgecolor='#333')
    ax.grid(alpha=0.3)
    plt.tight_layout()
    path2 = os.path.join(out_dir, "rope_ablation_trained_perwindow.png")
    plt.savefig(path2, dpi=220, bbox_inches='tight')
    plt.close()
    print(f"  Chart: {path2}")

    n_win = min(len(data_nope["windows"]), len(data_rope["windows"]))
    deltas = [data_nope["windows"][i]["mse"] - data_rope["windows"][i]["mse"] for i in range(n_win)]
    ...
    axes2[0].plot(pred_x, pred_nope, color='#d62728', linewidth=2, label='ICF Trained (noPE)',
    ...
    axes2[0].set_title(f'ICF Trained (noPE) — window {plot_idx} (MSE={w_nope["mse"]:,.0f})', fontweight='bold')
    ...
    axes2[1].plot(pred_x, pred_rope, color='#2ca02c', linewidth=2, label='ICF Trained (RoPE)',
    ...
    axes2[1].set_title(f'ICF Trained (RoPE) — window {plot_idx} (MSE={w_rope["mse"]:,.0f})', fontweight='bold')
    axes2[1].legend(loc='best', frameon=True, edgecolor='#333')
    axes2[1].grid(alpha=0.3)

    axes2[0].figure.suptitle(
        r"Same calendar window: index chosen for largest $\Delta\mathrm{MSE}$"
        r" (noPE $-$ RoPE; shows where RoPE helps most after training)",
        fontsize=9, y=1.03,
    )
    plt.tight_layout()
    path3 = os.path.join(out_dir, "rope_ablation_trained_forecast.png")
...
def main():
    parser = argparse.ArgumentParser(description="RoPE ablation on ICF-trained model")
    parser.add_argument(
        "--checkpoint", type=str, default=DEFAULT_CHECKPOINT,
        help="Path to ICF-trained .pt checkpoint (default: results/icf_trained/btc_usd_icf.pt)"
    )
    parser.add_argument(
        "--results_dir", type=str, default="results/rope_ablation_trained",
    ...
    print("Running ICF Trained with use_nope=True  (RoPE disabled)...")
    data_nope = run_icf_backtest(prices, use_nope=True,  label="ICF Trained (noPE)", checkpoint_path=args.checkpoint)

    print("\nRunning ICF Trained with use_nope=False (RoPE enabled)...")
    data_rope = run_icf_backtest(prices, use_nope=False, label="ICF Trained (RoPE)", checkpoint_path=args.checkpoint)
    ...
    plot_comparison(data_nope, data_rope, out_dir)

    print("\n" + "="*70)
    print("SUMMARY  (ICF Trained)")
    print("="*70)
    print(f"noPE:        MSE={summary['noPE']['mse']:>15,.0f}  MAE={summary['noPE']['mae']:>10,.1f}")
    print(f"RoPE:        MSE={summary['RoPE']['mse']:>15,.0f}  MAE={summary['RoPE']['mae']:>10,.1f}")
